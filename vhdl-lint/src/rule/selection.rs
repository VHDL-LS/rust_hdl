...
impl RuleOverrides {
    pub fn new(selections: Vec<RuleSelector>, ignores: Vec<RuleSelector>) -> RuleOverrides {
        RuleOverrides {
            ignores,
            selections,
        }
...
mod tests {
    ...
    fn from_args(selections: &[&str], ignores: &[&str]) -> RuleOverrides {
        let parse_all = |all: &[&str]| all.iter().map(|s| parse(s).unwrap()).collect::<Vec<_>>();
        RuleOverrides::new(parse_all(selections), parse_all(ignores))
    }
