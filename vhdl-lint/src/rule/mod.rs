...
use vhdl_syntax::{
    ...
    syntax::{validate::valid_node::Valid, AstNode, NodeKind, SyntaxNode},
};
...
pub struct AstRuleCtx<'a> {
    ...
    code: ErrorCode,
    // settings from the file belonging to this context
...
impl<'a> DiagnosticBuilder<'a> {
    ...
    pub fn with_note(self, note: impl Into<String>) -> DiagnosticBuilder<'a> {
        self.0.set_note(note);
        self
    }
}
...
impl<R: AstRule> ErasedAstRule for R {
    ...
    fn check_raw(&self, node: SyntaxNode, ctx: &mut AstRuleCtx) {
        self.check(
            &Valid::new_unchecked(<R::Node as AstNode>::cast_unchecked(node)),
            ctx,
        )
    }
